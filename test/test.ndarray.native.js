...
var dnanmskminabs = tryRequire( resolve( __dirname, './../lib/ndarray.native.js' ) );
var opts = {
	'skip': ( dnanmskminabs instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dnanmskminabs, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( dnanmskminabs.length, 7, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the minimum absolute value of a strided array according to a mask, ignoring NaN values', opts, function test( t ) {
	var mask;
	var x;
	var v;
	...
	v = dnanmskminabs( x.length, x, 1, 0, mask, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `NaN`', opts, function test( t ) {
	...
	v = dnanmskminabs( 0, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );

	v = dnanmskminabs( -1, x, 1, 0, mask, 1, 0 );
	t.strictEqual( isnan( v ), true, 'returns expected value' );
...
tape( 'the function supports `stride` parameters', opts, function test( t ) {
	...
	v = dnanmskminabs( 6, x, 2, 0, mask, 2, 0 );
...
tape( 'the function supports negative `stride` parameters', opts, function test( t ) {
	...
	v = dnanmskminabs( 6, x, -2, 10, mask, -2, 10 );
...
tape( 'the function supports `offset` parameters', opts, function test( t ) {
	...
	v = dnanmskminabs( 6, x, 2, 1, mask, 2, 1 );
	t.strictEqual( v, 1.0, 'returns expected value' );
